...
async def main():
    ...
    await index_documents_async(splitted_text, batch_size=500)

    log_header("PIPELINE COMPLETE")
    log_success("Documentation ingestion pipeline successfully completed!")
    log_info("Summary:")
    log_info(f"   - URLs mapped: ")
    log_info(f"   - Documents extracted: {len(all_docs)}")
    log_info(f"   - Chunks created: {len(splitted_text)}")
